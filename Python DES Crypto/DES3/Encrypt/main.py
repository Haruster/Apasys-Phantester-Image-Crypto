# Haruster DES3_Image_Encrypt.py
...
# DES3 암호화의 텍스트 공간은 16개의 정수 배수로 균등하게 나눌 수 없으므로 이를 채워야하며,
...
def encrypt_image_ecb(filename):
    ...
    value_encrypt = trans_format_RGB(des3_ecb_encrypt(key, pad(value_vector))[:imlength])
...
cv2.imread('image_encrypted_ecb.bmp')  # ecb 암호화가 진행된 이미지 열기
